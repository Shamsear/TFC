import { auth } from "@/lib/auth"
import { prisma } from "@/lib/prisma"
import { redirect, notFound } from "next/navigation"
import Link from "next/link"
import Image from "next/image"

export async function generateMetadata({ params }: { params: Promise<{ matchId: string }> }) {
  const { matchId } = await params
  const match = await prisma.matches.findUnique({
    where: { id: matchId },
    include: {
      homeTeam: { include: { team: true } },
      awayTeam: { include: { team: true } },
      tournament: true,
    },
  })

  return {
    title: `${match?.homeTeam.team.name} vs ${match?.awayTeam.team.name} | Match Details`,
    description: `Match details`,
  }
}

export default async function MatchDetailsPage({ params }: { params: Promise<{ matchId: string }> }) {
  const session = await auth()
  const { matchId } = await params

  if (!session?.user?.teamId) {
    redirect("/auth/signin")
  }

  // Get match info
  const match = await prisma.matches.findUnique({
    where: { id: matchId },
    include: {
      homeTeam: {
        include: {
          team: true,
        },
      },
      awayTeam: {
        include: {
          team: true,
        },
      },
      tournament: {
        include: {
          season: true,
        },
      },
      group: true,
    },
  })

  if (!match) {
    notFound()
  }

  // Check if user's team is involved
  const isHomeTeam = match.homeTeam.teamId === session.user.teamId
  const isAwayTeam = match.awayTeam.teamId === session.user.teamId
  const isInvolved = isHomeTeam || isAwayTeam

  // Determine match result for user's team
  let result: "won" | "draw" | "lost" | null = null
  if (match.status === "COMPLETED" && isInvolved) {
    if (match.homeScore === match.awayScore) {
      result = "draw"
    } else if (isHomeTeam) {
      result = (match.homeScore || 0) > (match.awayScore || 0) ? "won" : "lost"
    } else {
      result = (match.awayScore || 0) > (match.homeScore || 0) ? "won" : "lost"
    }
  }

  return (
    <div className="min-h-screen bg-[#0a0a0a] py-8">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Back Button */}
        <Link
          href="/team/matches"
          className="inline-flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-white transition-all mb-6"
        >
          ← Back to Matches
        </Link>

        {/* Match Header */}
        <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="text-sm text-gray-400 mb-1">
                {new Date(match.matchDate).toLocaleDateString("en-US", {
                  weekday: "long",
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })}
              </div>
              <div className="text-sm text-gray-400">
                {new Date(match.matchDate).toLocaleTimeString("en-US", {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="px-3 py-1 bg-[#E8A800]/10 border border-[#E8A800]/20 rounded-lg text-[#E8A800] text-sm">
                {match.tournament.name}
              </div>
              {match.group && (
                <div className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-gray-400 text-sm">
                  {match.group.name}
                </div>
              )}
            </div>
          </div>

          {/* Match Type & Status */}
          <div className="flex items-center gap-2 mb-6">
            <div className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-gray-400 text-sm">
              {match.matchType.replace(/_/g, " ")}
            </div>
            <div
              className={`px-3 py-1 rounded-lg text-sm font-medium ${
                match.status === "COMPLETED"
                  ? "bg-green-500/10 border border-green-500/20 text-green-400"
                  : match.status === "LIVE"
                  ? "bg-red-500/10 border border-red-500/20 text-red-400"
                  : match.status === "POSTPONED"
                  ? "bg-yellow-500/10 border border-yellow-500/20 text-yellow-400"
                  : "bg-blue-500/10 border border-blue-500/20 text-blue-400"
              }`}
            >
              {match.status}
            </div>
            {result && (
              <div
                className={`px-3 py-1 rounded-lg text-sm font-medium ${
                  result === "won"
                    ? "bg-green-500/10 border border-green-500/20 text-green-400"
                    : result === "draw"
                    ? "bg-yellow-500/10 border border-yellow-500/20 text-yellow-400"
                    : "bg-red-500/10 border border-red-500/20 text-red-400"
                }`}
              >
                {result === "won" ? "Victory" : result === "draw" ? "Draw" : "Defeat"}
              </div>
            )}
          </div>

          {/* Teams & Score */}
          <div className="grid grid-cols-3 gap-8 items-center">
            {/* Home Team */}
            <div className="text-center">
              {match.homeTeam.team.logoUrl && (
                <div className="relative w-24 h-24 mx-auto mb-3 rounded-xl overflow-hidden bg-white/5">
                  <Image
                    src={match.homeTeam.team.logoUrl}
                    alt={match.homeTeam.team.name}
                    fill
                    className="object-cover"
                  />
                </div>
              )}
              <h2
                className={`text-xl font-bold ${
                  isHomeTeam ? "text-[#E8A800]" : "text-white"
                }`}
              >
                {match.homeTeam.team.name}
              </h2>
              {isHomeTeam && (
                <div className="text-xs text-[#E8A800] mt-1">Your Team</div>
              )}
            </div>

            {/* Score */}
            <div className="text-center">
              {match.status === "COMPLETED" || match.status === "LIVE" ? (
                <div>
                  <div className="text-5xl font-bold text-white mb-2">
                    {match.homeScore ?? 0} - {match.awayScore ?? 0}
                  </div>
                  {(match.homePenalty !== null || match.awayPenalty !== null) && (
                    <div className="text-sm text-gray-400">
                      Penalties: {match.homePenalty ?? 0} - {match.awayPenalty ?? 0}
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-3xl font-bold text-gray-400">VS</div>
              )}
            </div>

            {/* Away Team */}
            <div className="text-center">
              {match.awayTeam.team.logoUrl && (
                <div className="relative w-24 h-24 mx-auto mb-3 rounded-xl overflow-hidden bg-white/5">
                  <Image
                    src={match.awayTeam.team.logoUrl}
                    alt={match.awayTeam.team.name}
                    fill
                    className="object-cover"
                  />
                </div>
              )}
              <h2
                className={`text-xl font-bold ${
                  isAwayTeam ? "text-[#E8A800]" : "text-white"
                }`}
              >
                {match.awayTeam.team.name}
              </h2>
              {isAwayTeam && (
                <div className="text-xs text-[#E8A800] mt-1">Your Team</div>
              )}
            </div>
          </div>
        </div>

        {/* Match Information */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          {/* Venue */}
          {match.venue && (
            <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6">
              <h3 className="text-lg font-bold text-white mb-3">Venue</h3>
              <div className="flex items-center gap-3">
                <div className="text-3xl">🏟️</div>
                <div className="text-white">{match.venue}</div>
              </div>
            </div>
          )}

          {/* Round */}
          {match.round && (
            <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6">
              <h3 className="text-lg font-bold text-white mb-3">Round</h3>
              <div className="flex items-center gap-3">
                <div className="text-3xl">📅</div>
                <div className="text-white">{match.round}</div>
              </div>
            </div>
          )}
        </div>

        {/* Tournament Info */}
        <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6 mb-6">
          <h3 className="text-lg font-bold text-white mb-4">Tournament Information</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <div className="text-gray-400 text-sm mb-1">Tournament</div>
              <div className="text-white font-medium">{match.tournament.name}</div>
            </div>
            <div>
              <div className="text-gray-400 text-sm mb-1">Season</div>
              <div className="text-white font-medium">{match.tournament.season.name}</div>
            </div>
            <div>
              <div className="text-gray-400 text-sm mb-1">Type</div>
              <div className="text-white font-medium">
                {match.tournament.tournamentType.replace(/_/g, " ")}
              </div>
            </div>
          </div>
        </div>

        {/* Notes */}
        {match.notes && (
          <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6">
            <h3 className="text-lg font-bold text-white mb-3">Match Notes</h3>
            <p className="text-gray-300">{match.notes}</p>
          </div>
        )}

        {/* Quick Actions */}
        <div className="mt-6 grid grid-cols-2 gap-4">
          <Link
            href={`/team/tournaments/${match.tournamentId}`}
            className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 hover:border-[#E8A800]/50 transition-all text-center"
          >
            <div className="text-2xl mb-2">🏆</div>
            <div className="text-white font-medium">View Tournament</div>
          </Link>
          <Link
            href="/team/matches"
            className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 hover:border-[#E8A800]/50 transition-all text-center"
          >
            <div className="text-2xl mb-2">⚽</div>
            <div className="text-white font-medium">All Matches</div>
          </Link>
        </div>
      </div>
    </div>
  )
}
