...
export async function POST(
  ...
  try {
    ...
    // Acquire lock (allow tiebreaker_pending to be finalized, and allow finalizing if force is true)
    ...
    if (round.roundType === 'bulk') {
      ...
      if (result.tieDetected && result.ties) {
        ...
        try {
          ...
          console.warn('[Push] Tiebreaker notification failed (non-fatal):', notifErr);
        }
      ...
      try {
        ...
        console.warn('[Push] Direct finalize win notification failed (non-fatal):', notifErr);
      }
