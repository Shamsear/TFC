...
export default function AdminFooterClient({ isSuperAdmin, activeSeasonId }: AdminFooterClientProps) {
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <div className="md:col-span-2">
            <Link href={isSuperAdmin ? "/super-admin" : "/sub-admin"} className="flex items-center gap-3 mb-4 group">
              ...
              <div>
                ...
                <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5">
                  {isSuperAdmin ? "Super Admin Workspace" : "Sub Admin Workspace"}
                </div>
              </div>
            </Link>
              ...
              {isSuperAdmin ? (
                <>
                  {[
                    ...
                    { label: 'Teams', href: '/super-admin/teams' },
                    { label: 'Seasons', href: '/super-admin/seasons' },
                    { label: 'Sub Admins', href: '/super-admin/sub-admins' }
                  ].map(link => (
                    <Link 
                      key={link.label} 
                      href={link.href} 
                ...
                <>
                  {[
                    ...
                    ...(activeSeasonId ? [
                      ...
                      { label: 'Calendar', href: `/sub-admin/${activeSeasonId}/calendar` }
                    ] : [])
                  ].map(link => (
                    <Link 
                      key={link.label} 
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md transition-all duration-300">
          <div className="relative w-full max-w-2xl max-h-[80vh] overflow-y-auto rounded-2xl bg-[#0d0d10]/95 border border-white/10 p-6 sm:p-8 shadow-[0_0_80px_rgba(0,0,0,0.8)] backdrop-blur-2xl animate-in fade-in zoom-in duration-200">
            ...
            <button
              ...
              className="absolute top-4 right-4 p-2 text-gray-400 hover:text-white rounded-lg bg-white/5 border border-white/10 transition-colors cursor-pointer"
            >
            ...
            <div className="mt-8 pt-4 border-t border-white/10 flex justify-end">
              <button
                onClick={() => setModalType(null)}
                className="px-6 py-2 bg-gradient-to-r from-[#E8A800] to-[#FFB347] hover:from-[#FFC93A] hover:to-[#FFB347] text-[#0a0a0a] rounded-xl font-bold transition-all shadow-md cursor-pointer"
              >
                Understood
              </button>
            </div>
