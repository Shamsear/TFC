...
export function PosterModal({
  ...
  const handleShare = async () => {
    if (!posterRef.current || sharing || downloading) return
    setSharing(true)
    try {
      ...
      if (
        ...
        await navigator.share({
          files: [file],
          title: type === 'swap' ? 'Swap Deal Poster' : 'Release Poster',
          text: whatsappMessage || undefined,
        })
      } else {
        ...
        alert('Poster downloaded & message copied to clipboard!\n\n(Direct image+text sharing is not supported by this browser, but the image was saved and message copied so you can easily attach it in WhatsApp).')
      }
    } catch (error: any) {
      if (error?.name !== 'AbortError') {
        console.error('Sharing failed:', error)
        alert('Sharing failed. Please use the Download Image button.')
      }
    } finally {
      setSharing(false)
    }
  ...
  const handleDownload = async () => {
    if (!posterRef.current || downloading || sharing) return
    setDownloading(true)
    try {
      const dataUrl = await captureTableAsPng(posterRef.current, {
        width: 800,
      ...
      <div className="bg-[#111111] border border-white/10 rounded-2xl p-6 max-w-5xl w-full flex flex-col md:flex-row gap-6 my-auto shadow-2xl">
        ...
        <div className="w-full md:w-80 flex flex-col justify-between gap-6">
          ...
          <div className="space-y-3 pt-5 border-t border-white/10">
            {/* Share Poster + Message Button */}
            <button
              onClick={handleShare}
              disabled={sharing || downloading}
              className="w-full px-4 py-3.5 bg-gradient-to-r from-[#25D366] to-[#128C7E] hover:from-[#20BA5A] hover:to-[#075E54] disabled:opacity-50 text-white rounded-xl font-black transition-all flex items-center justify-center gap-2 text-sm shadow-lg shadow-[#25D366]/20 cursor-pointer"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
              </svg>
              {sharing ? 'Preparing Share...' : 'Share Poster & Message'}
            </button>

            {whatsappMessage && copyToWhatsApp && (
              <button
                onClick={copyToWhatsApp}
                className="w-full px-4 py-2.5 bg-white/5 hover:bg-white/10 text-white rounded-xl font-bold transition-all flex items-center justify-center gap-2 text-xs border border-white/10 cursor-pointer"
              >
            ...
            <button
              ...
              className="w-full px-4 py-2.5 bg-[#E8A800] hover:bg-[#FFC93A] disabled:opacity-50 text-black rounded-xl font-bold transition-all flex items-center justify-center gap-2 text-xs shadow-lg shadow-[#E8A800]/10 cursor-pointer"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
              </svg>
              {downloading ? 'Capturing PNG...' : 'Download Image Only'}
            </button>

            <button
              onClick={onClose}
              className="w-full px-4 py-2.5 bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-semibold transition-all border border-white/5 cursor-pointer"
            >
              Close
            </button>
          </div>
