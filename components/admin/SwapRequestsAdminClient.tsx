...
export default function SwapRequestsAdminClient({
  ...
  const generateWhatsAppMessage = (request: Request) => {
    ...
    const requestingPlayersList = requestingPlayers.map(p => `- ${p.playerName} (£${(p.playerValue * 1000).toLocaleString()})`).join('\n')
    const targetPlayersList = targetPlayers.map(p => `- ${p.playerName} (£${(p.playerValue * 1000).toLocaleString()})`).join('\n')
