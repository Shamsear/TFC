...
• *Status:* APPROVED & ROSTERS UPDATED`
  ...
  const copyBestSwapPoll = () => {
    const approvedSwaps = filteredRequests.filter(r => r.status === 'approved')
    if (approvedSwaps.length === 0) {
      alert('No approved swaps found to generate a poll.')
      return
    }

    const options = approvedSwaps.map(request => {
      const requestingPlayers = request.players.filter(p => p.fromTeamId === request.requestingTeamId)
      const targetPlayers = request.players.filter(p => p.fromTeamId === request.targetTeamId)
      
      const reqNames = requestingPlayers.map(p => p.playerName.split(' ').pop()).join('&')
      const tgtNames = targetPlayers.map(p => p.playerName.split(' ').pop()).join('&')
      
      // WhatsApp poll options have a max length of 100 characters.
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-[10px] sm:text-xs font-black text-gray-500 uppercase tracking-widest font-mono">
            {seasonName} • Process player swap requests between teams
          </p>
        </div>
        
        <div className="flex items-center gap-3">
          <button
            onClick={copyBestSwapPoll}
            className="px-5 py-2.5 bg-[#25D366]/10 hover:bg-[#25D366]/20 border border-[#25D366]/30 text-[#25D366] rounded-xl font-bold transition-all text-xs uppercase tracking-wider cursor-pointer flex items-center gap-2"
            title="Copy WhatsApp Poll for Best Swap Deal"
          >
      ...
      <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-6 space-y-4 backdrop-blur-xl shadow-md mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1.5 flex-1">
            <label className="block text-[10px] text-gray-500 font-extrabold uppercase tracking-widest font-mono mb-2">Select Swap Window</label>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <h2 className="text-base sm:text-lg font-black text-white uppercase tracking-wider font-mono">Team Swap Statistics</h2>
                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest font-mono">View swap usage for all {teamStats?.length || 0} teams</p>
              </div>
            </div>
            <svg
