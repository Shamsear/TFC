...
export default function TransfersClient({ transfers, rounds, seasonId }: TransfersClientProps) {
  ...
  const filteredTransfers = useMemo(() => {
    ...
    return transfers.filter(t => t.roundId === selectedRound)
    ...
    <>
      ...
      {rounds.length > 0 && (
        <div className="mb-6">
          ...
          <select
            ...
            className="w-full sm:w-64 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-[#E8A800] focus:border-transparent"
          >
      ...
      {filteredTransfers.length === 0 ? (
        <div className="rounded-xl sm:rounded-2xl bg-white/5 border border-white/10 p-8 sm:p-12 text-center">
          ...
          <div className="text-lg sm:text-xl font-black text-white mb-2">No Transfers Found</div>
          <p className="text-[#D4CCBB] text-sm sm:text-base mb-6">
            {selectedRound === 'all' 
              ...
              : 'No transfers found for the selected round.'}
          </p>
          {selectedRound === 'all' && (
            <Link
