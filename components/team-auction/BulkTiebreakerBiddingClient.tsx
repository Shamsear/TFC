...
export default function BulkTiebreakerBiddingClient({
  ...
  const [submissionProgress, setSubmissionProgress] = useState({
    submitted: initialTiedTeams.filter(t => t.submitted).length,
    total: initialTiedTeams.length
  })
  ...
  const checkTiebreakerStatus = useCallback(async () => {
    try {
      const res = await fetch(`/api/team/bulk-tiebreakers/${tiebreaker.id}/status?t=${Date.now()}`, { cache: 'no-store' })
      if (res.ok) {
        ...
        if (data.participants) {
          setLiveTeams(prev => prev.map(t => {
            const fresh = data.participants.find((p: any) => p.teamId === t.teamId)
            return fresh ? { ...t, submitted: fresh.submitted, submittedAt: fresh.submittedAt } : t
          }))
        }
        
        // Update submission progress
        if (data.submissionProgress) {
          setSubmissionProgress(data.submissionProgress)
        }
      ...
      console.error('Failed to check tiebreaker status:', err)
    }
  }, [tiebreaker.id])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.resolutionInProgress) {
        ...
        router.refresh()
      } else if (data.tiebreakerResolved) {
        // Tiebreaker resolved right away
  ...
  const handleWithdraw = async () => {
    if (myBid?.submitted) {
      setMessage({ 
        type: 'error', 
        text: 'You cannot withdraw after submitting your bid.' 
      })
      return
    }
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Withdrawn successfully. Redirecting...' })
      ...
      <div className="border-b border-white/5 bg-white/[0.02] backdrop-blur-md mb-6 relative z-10 shadow-lg shadow-black/20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div className="rounded-xl bg-white/[0.02] border border-white/5 hover:border-[#E8A800]/30 p-4 transition-all duration-300 backdrop-blur-md shadow-lg shadow-black/20 group">
              <div className="text-[10px] text-[#7A7367] mb-1 font-bold uppercase tracking-wider transition-colors group-hover:text-[#D4CCBB]">Base Price</div>
              <div className="text-lg sm:text-xl font-black text-[#E8A800] drop-shadow-[0_0_8px_rgba(232,168,0,0.2)] font-mono">£{tiebreaker.basePrice.toLocaleString()}</div>
            </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 relative z-10">
        ...
        <div className="mb-6 rounded-2xl bg-purple-950/10 border border-purple-500/20 p-5 backdrop-blur-md relative overflow-hidden">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-base font-extrabold text-white uppercase tracking-wide mb-1">Sealed Bid Tiebreaker</h3>
              <p className="text-sm text-purple-200 font-medium leading-relaxed">
                Multiple teams are currently tied for {tiebreaker.basePlayer.name}. Submit your final sealed bid amount below. All bids remain completely private until everyone has submitted. The highest bid captures the player.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 rounded-2xl bg-white/[0.01] border border-white/5 p-5 sm:p-6 backdrop-blur-md shadow-xl">
          ...
          <div className="mt-5 pt-4 border-t border-white/5 flex items-center justify-between text-xs font-semibold text-[#7A7367]">
            ...
            <span className="font-extrabold text-white text-sm font-mono">
              {submissionProgress.submitted} <span className="text-white/40">/</span> {submissionProgress.total} SUBMITTED
            </span>
          </div>
        </div>
        ...
        {!myBid?.submitted && tiebreaker.status === 'active' && (
          <div className="rounded-2xl bg-white/[0.01] border border-white/5 p-5 sm:p-6 backdrop-blur-md shadow-xl">
            <h2 className="text-base font-extrabold text-white mb-4 uppercase tracking-wider">Submit Your Bid</h2>
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2.5 mt-4">
                ...
                <button
                  onClick={() => setNewBidAmount(prev => Math.min(prev + 100, maxBidLimit))}
                  disabled={newBidAmount + 100 > maxBidLimit}
                  className="flex-1 px-4 py-2.5 rounded-xl bg-white/[0.02] border border-white/10 text-white hover:bg-white/5 hover:border-[#E8A800]/50 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-extrabold text-xs uppercase tracking-wider cursor-pointer"
                >
                  +£100
                </button>
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                disabled={myBid?.submitted}
                className="px-6 py-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed font-extrabold text-xs uppercase tracking-wider cursor-pointer shadow-lg shadow-red-950/10"
                title={myBid?.submitted ? 'Cannot withdraw after submission' : 'Withdraw from tiebreaker'}
              >
                Withdraw
              </button>
            </div>
        ...
        {myBid?.submitted && (
          <div className="rounded-2xl bg-emerald-500/[0.03] border border-emerald-500/20 p-8 text-center backdrop-blur-md shadow-2xl relative overflow-hidden">
            ...
            <div className="p-4 rounded-xl bg-white/[0.01] border border-white/5 flex items-center justify-between text-xs font-bold text-[#7A7367] max-w-sm mx-auto uppercase">
              ...
              <span className="font-extrabold text-white text-sm font-mono">
                {submissionProgress.submitted} / {submissionProgress.total} teams submitted
              </span>
            </div>
          </div>
        )}
