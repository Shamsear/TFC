...
export function AchievementsClient({ team }: AchievementsClientProps) {
  ...
  return (
    <div className="min-h-screen bg-[#070708] text-white py-12 px-4 md:px-8 font-sans selection:bg-[#7c3aed] selection:text-white">
      <div className="max-w-6xl mx-auto">
        
        {/* Back Link to Team Profile */}
        <a 
          href={`/teams/${team.id}`}
          className="inline-flex items-center gap-2 mb-8 text-sm text-gray-400 hover:text-cyan-400 transition-colors duration-200"
        >
        ...
        <div className="relative rounded-2xl bg-white/[0.02] border border-white/5 backdrop-blur-xl p-6 md:p-8 mb-8 overflow-hidden shadow-[0_0_80px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
            ...
            {/* Quick Stats Cabinet */}
            <div className="flex gap-4 border-t border-white/5 pt-6 md:pt-0 md:border-t-0 md:border-l md:pl-8 w-full md:w-auto justify-around md:justify-start">
              <div className="text-center">
                <div className="text-3xl font-extrabold text-cyan-400 font-mono">{totalEarned}</div>
                <div className="text-[10px] uppercase tracking-wider text-gray-400 mt-1">Earned Badges</div>
              </div>
              ...
              <div className="text-center">
                ...
                <div className="text-[10px] uppercase tracking-wider text-gray-400 mt-1">Completion</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-8 border-b border-white/5 pb-4">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 flex items-center gap-2 border cursor-pointer ${
              activeTab === 'trophies'
                ? 'bg-amber-500/10 text-amber-400 border-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.1)]'
                : 'bg-white/[0.01] text-gray-400 border-white/5 hover:text-white hover:bg-white/[0.03]'
            }`}
          >
        ...
        {activeTab === 'trophies' ? (
          <>
            ...
            <div className="mb-6 flex items-center justify-between animate-[fadeIn_0.3s_ease-out]">
              <h2 className="text-xl font-bold tracking-tight text-white flex items-center gap-3">
                ...
                Trophy Room Showcase
                <span className="text-xs font-normal text-gray-500 font-mono">({totalEarned}/{allBadges.length} unlocked)</span>
              </h2>
            </div>

            {/* Badges Grid */}
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 animate-[fadeIn_0.3s_ease-out]">
              {allBadges.map(badge => {
                ...
                return (
                  <div 
                    ...
                    className={`relative group rounded-xl p-5 border text-center transition-all duration-300 select-none cursor-pointer flex flex-col items-center justify-between ${
                      unlocked 
                        ? 'bg-white/[0.01] hover:bg-white/[0.03] border-white/10 hover:-translate-y-1' 
                        : 'bg-black/40 border-white/[0.03]'
                    }`}
                    style={{ 
                      boxShadow: unlocked ? `0 0 20px ${glowColor}10` : 'none',
                    }}
                  >
                    ...
                    <div className="mb-4 relative h-16 w-16 flex items-center justify-center">
                      ...
                      {unlocked && (
                        <div 
                          className="absolute inset-0 blur-lg rounded-full opacity-0 group-hover:opacity-40 transition-opacity duration-300 pointer-events-none"
                    ...
                    <div className="w-full">
                      ...
                      <p className="text-[10px] text-gray-500 mt-1 line-clamp-1 leading-normal">
          ...
          <div className="rounded-2xl bg-white/[0.01] border border-white/5 backdrop-blur-xl p-6 md:p-8 shadow-[0_0_80px_rgba(0,0,0,0.5)] animate-[fadeIn_0.3s_ease-out]">
            <div className="mb-6">
              <h2 className="text-xl font-bold tracking-tight text-white flex items-center gap-3">
                <svg className="w-6 h-6 text-cyan-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
                </svg>
                XP Progression Ledger
                <span className="text-xs font-normal text-gray-500 font-mono">({team.xpHistory.length} ledger logs tracked)</span>
              </h2>
            ...
            {team.xpHistory.length === 0 ? (
              ...
              <div className="overflow-x-auto rounded-xl border border-white/5 bg-black/40">
                <table className="w-full border-collapse text-left text-sm text-gray-300 font-sans">
                  <thead className="bg-white/[0.02] text-xs uppercase tracking-wider text-gray-400 border-b border-white/5">
                    <tr>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {team.xpHistory.map(log => {
                      ...
                      if (log.xpType === 'MATCH_WON') typeLabelColor = 'text-emerald-400 bg-emerald-950/20 border-emerald-500/20';
                      if (log.xpType === 'GOAL_SCORED') typeLabelColor = 'text-cyan-400 bg-cyan-950/20 border-cyan-500/20';
                      if (log.xpType === 'BADGE_UNLOCKED') typeLabelColor = 'text-amber-400 bg-amber-950/20 border-amber-500/20';
                      if (log.xpType === 'MATCH_PLAYED') typeLabelColor = 'text-blue-400 bg-blue-950/20 border-blue-500/20';

                      return (
                        <tr key={log.id} className="hover:bg-white/[0.01] transition-colors duration-150">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${typeLabelColor}`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right font-bold font-mono text-cyan-400">
                            +{log.amount} XP
                          </td>
            ...
            >
              ...
              <div className="flex flex-col items-center text-center relative z-10 pt-4">
                ...
                <div className="w-full bg-white/[0.01] border border-white/5 rounded-xl p-4 mb-6">
                  <h4 className="text-[10px] text-gray-500 uppercase tracking-widest text-left mb-1">Requirement</h4>
