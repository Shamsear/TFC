...
function AdminTableSnapshot({
  ...
  const posStyle = (idx: number) =>
    idx === 0 ? { background: 'linear-gradient(135deg, #FFD700, #FFA500)', color: '#0a0a0a' } :
    idx === 1 ? { background: 'linear-gradient(135deg, #E0E0E0, #9E9E9E)', color: '#0a0a0a' } :
    idx === 2 ? { background: 'linear-gradient(135deg, #CD7F32, #8B4513)', color: '#0a0a0a' } :
    ...
    <>
      <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
        ...
        <button
          ...
          className={`flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl font-bold text-xs sm:text-sm border transition-all hover:scale-[1.02] disabled:opacity-60 cursor-pointer whitespace-nowrap ${
            shareDone
