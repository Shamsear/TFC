              ...
              <div className="rounded-2xl bg-dark-100 border border-white/5 p-6 shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    {[
                      { icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>, value: Math.round(standings.reduce((sum, s) => sum + (s.played || 0), 0) / 2), label: 'Matches Played', color: 'bg-blue-500/[0.02] border-blue-500/10 text-blue-400' },
                      { icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg>, value: standings.reduce((sum, s) => sum + (s.goalsFor || 0), 0), label: 'Total Goals', color: 'bg-emerald-500/[0.02] border-emerald-500/10 text-emerald-400' },
                      { icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>, value: (standings.reduce((sum, s) => sum + (s.played || 0), 0) > 0 ? (standings.reduce((sum, s) => sum + (s.goalsFor || 0), 0) / (standings.reduce((sum, s) => sum + (s.played || 0), 0) / 2)).toFixed(1) : '0.0'), label: 'Avg Goals / Match', color: 'bg-[#E8A800]/[0.02] border-[#E8A800]/10 text-[#E8A800]' },
                      { icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>, value: teams.reduce((sum, t) => sum + (t.cleanSheets || 0), 0), label: 'Clean Sheets', color: 'bg-purple-500/[0.02] border-purple-500/10 text-purple-400' },
                      ...
                      <div key={label} className={`relative rounded-2xl p-4 text-center backdrop-blur-xl shadow-md overflow-hidden ${color} border`}>
                        <div className="text-blue-400 mb-1 flex justify-center">{icon}</div>
                      ...
                      return (
                        <div className="flex items-center justify-between p-3.5 rounded-xl bg-white/[0.01] border border-white/5 hover:border-white/10 transition-all">
                          <div className="flex items-center gap-3">
                            <svg className="w-5 h-5 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
                            <div>
                ...
                <div key={groupName} className="rounded-2xl bg-dark-100 border border-white/5 overflow-hidden shadow-lg">
                  ...
                  <div className="p-2 space-y-1">
                    {rows.slice(0, 5).map((s, idx) => {
                      ...
                      return (
                        <Link
                          href={s.managerId ? `/managers/${s.managerId}` : '#'}
                          key={s.id}
                          className="flex items-center gap-2.5 px-2.5 py-2 rounded-xl border border-transparent hover:bg-white/[0.03] transition-all cursor-pointer"
                        >
