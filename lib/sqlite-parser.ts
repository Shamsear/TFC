/**
 * SQLite database parser for eFootball player data
 * Reads player data from efootball_latest.db format
 */
...
export interface EFootballPlayer {
  ...
  // Dribbling Skills
  scissorsFeint?: string;
  doubleTouch?: string;
  flipFlap?: string;
  marseilleTurn?: string;
  sombrero?: string;
  chopTurn?: string;
  cutBehindTurn?: string;
  scotchMove?: string;
  soleControl?: string;
  momentumDribbling?: string;
  accelerationBurst?: string;
  magneticFeet?: string;
  ...
  // Shooting Skills
  longRangeCurler?: string;
  blitzCurler?: string;
  chipShotControl?: string;
  knuckleShot?: string;
  dippingShot?: string;
  risingShot?: string;
  longRangeShooting?: string;
  lowScreamer?: string;
  acrobaticFinishing?: string;
  heelTrick?: string;
  firstTimeShot?: string;
  phenomenalFinishing?: string;
  willpower?: string;
  
  // Passing Skills
  oneTouchPass?: string;
  throughPassing?: string;
  weightedPass?: string;
  pinpointCrossing?: string;
  edgedCrossing?: string;
  outsideCurler?: string;
  rabona?: string;
  noLookPass?: string;
  gameChangingPass?: string;
  visionaryPass?: string;
  phenomenalPass?: string;
  lowLoftedPass?: string;
  ...
  longThrow?: string;
  gkLongThrow?: string;
  penaltySpecialist?: string;
  gkPenaltySaver?: string;
  gkDirectingDefence?: string;
  gkSpiritRoar?: string;
  ...
  interception?: string;
  blocker?: string;
  aerialSuperiority?: string;
  slidingTackle?: string;
  longReachTackle?: string;
  fortress?: string;
  acrobaticClearance?: string;
  aerialFort?: string;
  ...
  superSub?: string;
  fightingSpirit?: string;
  trickster?: string;
  mazingRun?: string;
  speedingBullet?: string;
  incisiveRun?: string;
 ...
 * Now using detailed positions: GK, CB, LB, RB, DMF, CMF, LMF, RMF, AMF, SS, LWF, RWF, CF
 */
function mapPosition(efootballPosition: string): string {
  const pos = efootballPosition.toUpperCase();
  
  // Direct mapping for supported positions
  const validPositions = ['GK', 'CB', 'LB', 'RB', 'DMF', 'CMF', 'LMF', 'RMF', 'AMF', 'SS', 'LWF', 'RWF', 'CF'];
  if (validPositions.includes(pos)) {
    return pos;
  }
  
  // Map similar positions
  if (pos === 'LWB') return 'LB';
  if (pos === 'RWB') return 'RB';
  
  // Default fallback to CMF (central midfielder)
  return 'CMF';
}

/**
 * Parse SQLite database buffer and extract player data
 */
export function parseSQLiteDB(buffer: Buffer): ParseResult {
  let tempPath: string | null = null;
  let db: Database.Database | null = null;
  
  try {
    // Write buffer to temporary file
    tempPath = path.join(os.tmpdir(), `efootball-${Date.now()}.db`);
